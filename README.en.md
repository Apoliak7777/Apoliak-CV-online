...
- 🌌 **Design layer** — on top of the base system sits a layer of atmosphere and detail that changes no layout: three blurred colour blobs (`.aurora`) drift slowly behind the content, a fine grain runs across the whole page (`body::after` with SVG noise), a three-colour ring rotates around the monogram, cards get gradient 1px borders via `mask-composite: exclude` (lighter top-left, into the accent bottom-right; an accent border cross-fades in on hover), each stat carries its own palette colour, the scrollbar is dark and a gradient hairline sits above the footer. It all lives in `@supports`, so an older browser without `mask-composite` gets a plain 1px line; in print and under `prefers-reduced-motion` the layer steps back — the blobs and grain disappear, the ring stops, the borders return to a solid line.
